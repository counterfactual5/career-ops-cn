...
<!-- ============================================================
     ...
     你的个性化配置应写入 modes/_profile.md 中（不会被项目更新覆盖）。
     ============================================================ -->
...
2. 在每个会话的第一次评估前运行 `node scripts/diagnosis/cv-sync-check.mjs`。如果发现同步警告，提示候选人。
...
7. 生成内容与 JD 的语言保持一致（默认使用中文，英文 JD 使用英文）。
...
9. **向 tracker 添加新记录时必须使用 TSV 格式** -- 严禁直接编辑 `applications.md`，将 TSV 文件写入 `batch/tracker-additions/` 目录，由 `node scripts/tracker/merge-tracker.mjs` 统一合并。
10. **在每份评估报告的头部，如果候选人提供了 URL，包含 `**URL:**` 字段；如果仅提供截图或粘贴文本，可留空或填入 `**Source:** 截图/粘贴`。**
