...
2. 在每个会话的第一次评估前运行 `node scripts/cv-sync-check.mjs`。如果发现同步警告，提示候选人。
...
9. **向 tracker 添加新记录时必须使用 TSV 格式** -- 严禁直接编辑 `applications.md`，将 TSV 文件写入 `batch/tracker-additions/` 目录，由 `node scripts/merge-tracker.mjs` 统一合并。
