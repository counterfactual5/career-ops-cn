...
当候选人通过**截图/照片**或**粘贴文本**提供职位描述（JD）时，**始终**输出以下七个维度的深度评估结果（A-F 评估 + G 真实性）：

> **重要：本项目不依赖 Playwright 或 URL 抓取。** 国内招聘平台（BOSS直聘、拉勾、猎聘、智联等）反爬激进，一律需要登录才能查看完整 JD。所有 JD 均由候选人主动截图/粘贴提供——这意味着 JD 质量由候选人把关，不会被垃圾信息淹没。

## 步骤 0 — 岗位画像识别
首先读取候选人提供的截图（使用 Read 工具的图片识别能力提取文本）或直接读取粘贴的 JD 文本。将该职位划分为 6 种画像之一（参见 `_shared.md`）。如果是混合型，标明最接近的 2 种。这决定了：
...
**中国大陆市场特有信号分析：**
1. **从 JD 文本/截图中提取的时效信号：**
   - 截图中的"活跃/今日活跃/刚刚在线"状态（如 BOSS直聘界面上的在线状态标识）。
   - JD 文本中是否包含招聘人数、紧急程度等时效性描述。
   - 候选人本人是否注意到该岗位已挂出一段时间（可主动询问候选人）。
2. **职位描述的具体与详实度：**
...
3. **外围招聘信号** (通过 2-3 个 WebSearch 查询)：
   - 查询 `"{公司名称}" 裁员` 或 `"{公司名称}" 劳动纠纷`。
   - 查询该公司的近期融资状态或经营危机报道。
   - 若近期发生大规模裁员，评估该岗位所属部门是否受到波及。
4. **历史重合度检测** (如果存在 `scan-history.tsv`)：
   - 检查该公司是否在过去 90 天内重复出现类似职能的岗位。
