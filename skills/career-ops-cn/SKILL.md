...
> **路径说明：** `{baseDir}` 指向 `skills/career-ops-cn/`，向上两层回到项目根目录。OpenClaw 会解析此占位符。
...
用户可以通过以下斜杠命令（OpenClaw 自动注册）快速触发特定功能：
...
```bash
# 从 GitHub 安装
openclaw skills install git:counterfactual5/career-ops-cn --as career-ops-cn

# 从本地目录安装
openclaw skills install /path/to/career-ops-cn --as career-ops-cn
```
