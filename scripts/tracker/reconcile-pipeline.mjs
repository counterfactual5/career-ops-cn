...
if (process.argv.includes("-h") || process.argv.includes("--help")) {
  console.log(
    "Usage: node reconcile-pipeline.mjs [--dry-run] [--state <path>] [--pipeline <path>]",
  );
  console.log(
...
function resolveInsideRepo(inputPath, fallbackPath, flag) {
  ...
  try {
    ...
    targetReal = existsSync(abs)
      ? realpathSync(abs)
      : realpathSync(dirname(abs));
  } catch {
  ...
  if (rel.startsWith("..") || isAbsolute(rel)) {
    console.error(
      `Invalid ${flag}: path must stay inside the repository (${abs})`,
    );
    process.exit(1);
  }
...
const PIPELINE_FILE = resolveInsideRepo(
  argValue("--pipeline"),
  defaultPipeline,
  "--pipeline",
);
...
let reportFiles = [];
try {
  reportFiles = readdirSync(REPORTS_DIR).filter((f) => f.endsWith(".md"));
} catch {
  /* no reports dir */
}
...
for (let i = pendStart + 1; i < pendEnd; i++) {
  ...
  movedProcLines.push(
    `- [x] ${reportLink} | ${url} | ${company} | ${role} | ${score} | PDF ${pdf}`,
  );
...
if (procStart < 0 && movedProcLines.length > 0) {
  const processedHeader = /Pending/i.test(lines[pendStart])
    ? "## Processed"
    : "## Procesadas";
...
console.log(
  `🔄 ${realMoves.length} processed entr${realMoves.length === 1 ? "y" : "ies"} moved Pendientes → Procesadas:`,
);
for (const m of realMoves)
  console.log(`   + #${m.num} ${m.company} — ${m.role} (${m.score})`);
const dups = moved.filter((m) => m.dup);
if (dups.length)
  console.log(
    `🧹 ${dups.length} stale Pendientes entr${dups.length === 1 ? "y" : "ies"} dropped (already in Procesadas).`,
  );
console.log(
  `📋 Pendientes now: ${newCount} entr${newCount === 1 ? "y" : "ies"}`,
);
...
console.log(
  `✅ pipeline.md updated (backup: ${PIPELINE_FILE}.pre-reconcile.bak)`,
);
