...
if (!applyUrl || !pdfPath) {
  console.error(
    "Usage: node prepare-application.mjs --url <apply_url> --pdf <pdf_path> [--cover <cover_txt>]",
  );
  process.exit(1);
}
...
if (!absPdf.startsWith(outputDir + "/") && absPdf !== outputDir) {
  console.error(
    `Error: --pdf must point to a file inside output/ (got ${pdfPath})`,
  );
  process.exit(1);
}
...
function buildGreenhouseFields(profile, cover, pdfFile) {
  return [
    ...
    cover
      ? [
          "cover_letter",
          `${cover.wordCount} words — ${cover.text.slice(0, 80).replace(/\n/g, " ")}…`,
        ]
      : null,
...
function buildAshbyFields(profile, cover, pdfFile) {
  return [
    ...
    ["resume", `${pdfFile}  ← attach this file`],
    cover
      ? ["coverLetter", `(${cover.wordCount} words — paste from cover file)`]
      : null,
...
function buildLeverFields(profile, cover, pdfFile) {
  return [
    ...
    ["resume", `${pdfFile}  ← attach this file`],
    cover
      ? ["comments", `(${cover.wordCount} words — paste from cover file)`]
      : null,
...
if (!detected) {
  console.error(
    `Error: URL not recognized as Greenhouse, Ashby, or Lever.\n  URL: ${applyUrl}`,
  );
  process.exit(1);
}
...
let fields;
if (ats === "greenhouse")
  fields = buildGreenhouseFields(profile, cover, pdfFile);
...
for (const [key, value] of fields) {
  console.log(
    `  ${key.padEnd(labelWidth)}${value || "(not set — check config/profile.yml)"}`,
  );
}
