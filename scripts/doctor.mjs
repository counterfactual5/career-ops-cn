...
function checkPipelineFile() {
  ...
  return { pass: true, label: 'data/pipeline.md 尚未创建（首次使用时自动生成）' };
}
...
async function main() {
  ...
  if (JSON_OUT) {
    const result = {
      ...
      checks: checks.map(({ pass, label, fix }) => ({ pass, label, fix: fix ?? null })),
    };
  ...
  if (failed.length === 0) {
    console.log(`  ${green('全部通过')}${warnings.length ? `（${warnings.length} 个警告）` : ''}\n`);
    process.exit(0);
  } else {
