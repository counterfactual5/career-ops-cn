    ...
    counts: {
      ...
      totalBullets: (() => {
        const ex = Array.isArray(payload.experience)
          ...
          : [];
        const pr = Array.isArray(payload.projects)
...
async function runSelfTest() {
  ...
  if (unresolved) {
    console.error(
      `Self-test failed: unresolved placeholders: ${[...new Set(unresolved)].join(", ")}`,
    );
    process.exit(1);
  }
  ...
  const report = {
    ...
    counts: {
      ...
      totalBullets: (() => {
        const ex = Array.isArray(sample.experience)
          ? sample.experience.flatMap((e) =>
              Array.isArray(e?.bullets) ? e.bullets : [],
            )
          : [];
        const pr = Array.isArray(sample.projects)
          ? sample.projects.flatMap((p) =>
              Array.isArray(p?.bullets) ? p.bullets : [],
            )
          : [];
        return ex.length + pr.length;
