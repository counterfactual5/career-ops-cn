...
function validateCvSectionOrder(html, cvMarkdown) {
  ...
  const sourcePositions = new Map(
    source.map((section, index) => [section.key, index]),
  );
  const renderedComparable = rendered.filter((section) =>
    sourcePositions.has(section.key),
  );
  ...
  for (let i = 1; i < renderedComparable.length; i++) {
    ...
    if (sourcePositions.get(current.key) < sourcePositions.get(previous.key)) {
      const renderedOrder = renderedComparable
        .map((section) => section.title)
        .join(" -> ");
      const sourceOrder = source
        .filter((section) =>
          renderedComparable.some(
            (renderedSection) => renderedSection.key === section.key,
          ),
        )
        .map((section) => section.title)
        .join(" -> ");
...
function updatePDFManifest(reportNum, pdfPath, htmlPath, format) {
  ...
  if (existsSync(manifestPath)) {
    lines = readFileSync(manifestPath, "utf-8")
      .split("\n")
      .filter((line) => {
...
async function generatePDF() {
  ...
  if (!inputPath || !outputPath) {
    console.error(
      "Usage: node generate-pdf.mjs <input.html> <output.pdf> [--format=letter|a4] [--report=NNN]",
    );
    process.exit(1);
  }
  ...
  if (reportNum && !/^\d+$/.test(reportNum)) {
    console.error(
      `Invalid --report "${reportNum}". Use the numeric tracker/report number, e.g. --report=018`,
    );
    process.exit(1);
  }
  ...
  if (relOut === "" || relOut.startsWith("..") || isAbsolute(relOut)) {
    console.error(
      `Refusing to write the PDF outside the project directory: ${outputPath}`,
    );
    process.exit(1);
  }
...
export async function inlineLocalFonts(html) {
  ...
  for (const name of names) {
    ...
    try {
      ...
      dataUrls.set(
        name,
        `url('data:${MIME[ext] || "application/octet-stream"};base64,${buf.toString("base64")}')`,
      );
    } catch (err) {
      if (err?.code !== "ENOENT") throw err;
      console.warn(
  ...
  return html.replace(
    FONT_REF,
    (match, _quote, name) => dataUrls.get(name) || match,
  );
}
...
export async function renderHtmlToPdf(html, outputPath, opts = {}) {
  ...
  const tmpHtmlPath = resolve(
    baseDir,
    `.career-ops-render-${randomUUID()}.html`,
  );
  ...
  try {
    ...
    try {
      ...
      console.log(
        `🔗 Manifest: data/pdf-index.tsv updated${reportNum ? ` (report ${reportNum})` : " (no --report given)"}`,
      );
    } catch (err) {
