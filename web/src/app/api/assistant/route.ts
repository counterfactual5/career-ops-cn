...
ACTIONS:
- navigate {"path":"/pipeline?tab=OFFER&min=4"} — take the user to a section. Valid paths: /, /pipeline, /analytics, /cv, /config, /apply, /pipeline/{n} (a report), /jobs/{id} (a worker). The path may carry a query string.
...
- setProfile {"name":"…","email":"…","location":"…","roles":["AI Engineer","ML Engineer"],"compMin":70000,"compMax":95000,"currency":"EUR","remote":"Remote (EU)","seniority":"Senior"} — PROPOSE the user's profile; the app shows a confirm card and ONLY on their OK writes config/profile.yml (merge-safe — it never clobbers their other fields). Emit only fields you're confident about (most come from their CV). NEVER write a profile they didn't approve.
...
2. WOW #1 — EVALUATE. The moment you have a CV, ask the user to paste or screenshot a JD they're interested in (国内招聘平台需登录，不能抓取，必须用户提供截图或粘贴文本). When they send one, immediately run evaluate on it. A first scored job with explanation is the aha trigger.
3. Then DEEPEN, value-interleaved. Now that they've seen a score, confirm targeting so future evaluations sharpen: ask for roles, then comp, then location — one or two at a time, ~2–3 minutes, encouraging.
...
export async function POST(req: Request) {
  ...
  if (!message || !cliId) {
    return new Response(
      JSON.stringify({ error: "message and cliId required" }),
      { status: 400 },
    );
  }
  ...
  if (!resolved) {
    return new Response(
      JSON.stringify({ error: `CLI '${cliId}' not found on this machine` }),
      {
        status: 404,
  ...
  const setupLine = onboardingNeeded
    ...
    : `\n\nSETUP STATE: this user is fully set up (CV + profile all on file). Do NOT run onboarding or ask for a CV — just help them with what they actually asked.`;
